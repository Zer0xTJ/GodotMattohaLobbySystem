...
signal server_rpc_received(method_name: String, payload: Dictionary, sender: int)

## Emmited on client when a client rpc received.
## [br][param method_name] RPC method name.
## [br][param payload] payload as godot dictionaty.
## [br][param sender] peer id who send the rpc.
signal client_rpc_received(method_name: String, payload: Dictionary, sender: int)
...
## MattohaClient instance that is used to interact with MattohaServer.
var client: MattohaClient
## MattohaServer instance.
var server: MattohaServer

func _enter_tree():
	client = MattohaClient.new()
	add_child(client)
	server = MattohaServer.new()
	add_child(server)
	MattohaSystem.ClientRpcReceived.connect(_on_client_rpc_received)
	MattohaSystem.ServerRpcReceived.connect(_on_server_rpc_received)

func _on_client_rpc_received(method_name: String, payload: Dictionary, sender: int):
	emit_signal("client_rpc_received", method_name, payload, sender)

func _on_server_rpc_received(method_name: String, payload: Dictionary, sender: int):
	emit_signal("server_rpc_received", method_name, payload, sender)
...
## Send reliable server RPC to server, this will emmit "server_rpc_received" on server node.
## [br][param method_name] RPC method name.
## [br][param payload] payload as godot dictionaty.
...
## Send reliable client RPC to peer, this will emmit "client_rpc_received" on client node.
...
func is_node_owner(node: Node) -> bool:
